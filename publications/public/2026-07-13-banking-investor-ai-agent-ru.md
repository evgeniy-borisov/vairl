...
Фактура для go / no-go. Три ответа — и можно решать, идём ли в это.

1. **Что делать?** Построить **AI Wealth Management Platform**: единый Customer 360 + мультиагентный оркестратор (портфель, цели, риски, аналитика, мониторинг, compliance) + детерминированные solvers (оптимизация, риск, атрибуция, логрег/скоры под NPV) + LLM только для диалога и объяснений. Не «чат-бот про инвестиции», а контур ведения клиента к цели с проактивным мониторингом.

2. **Зачем?** Поднять **NPV отношений с клиентом**: удержание после просадок, рост AUM и fee, дешевле и точнее cost-to-serve, меньше mis-selling и регуляторного хвоста. Клиентский DCF цели («квартира к 2030») и банковский DCF LTV — одна экономика; ассистент без этой оптики — cost center.

3. **Каким образом?** Классифицировать запрос до agent loop → нужный агент и pipeline → решение считает математика (Markowitz / Monte Carlo / risk / scorecard) → Compliance Agent (suitability, audit trail) → Communication Agent объясняет. События (Kafka/CEP) держат план в фоне. Автономные сделки без human approve — вне периметра.
...
0. [Решение на один слайд](#decision-slide) — что / зачем / каким образом
