---
layout: post
title: "Как я собирал инвестиционного агента в BlankChain: от NPV до синтетики и Kaggle"
date: 2026-07-13 18:00:00 +0300
excerpt: "Личный опыт сборки AI Wealth-ассистента в BlankChain: мультиагенты под комплаенсом, логреги и NPV как первопричина пайплайнов, процесс разработки и как тестировать агента на синтетике и датасетах с Kaggle."
lang: ru
...
В BlankChain я собирал не «чат про акции», а **AI Wealth Management Platform**: Customer 360, оркестратор агентов, детерминированные solvers и LLM только для объяснений. Бизнес-заказ звучал просто — поднять NPV клиента; инженерия оказалась сложнее: комплаенс, audit trail и запрет отдавать сделки нейросети.
...
Фактура для go / no-go — так я и защищал инициативу у нас в BlankChain.
...
1. [Как это началось в BlankChain](#blankchain-story)
...
## Как это началось в BlankChain {#blankchain-story}

В BlankChain ко мне пришли не с запросом «сделай GPT для инвесторов». Пришли с болью: после просадки клиент открывает приложение, пишет «продавать?», оператор отвечает шаблоном, человек либо уходит, либо нажимает всё продать. Fee падает, LTV падает, колл-центр горит.
...
Дальше — как мы это собирали: продукт, процесс, архитектура, и отдельно — как тестировали агента, когда прод-витрины ещё нельзя было трогать. Примеры ниже — из контура BlankChain (имена клиентов и цифры обезличены/синтетичны там, где нужно).

<figure style="margin: 2em auto; text-align: center;">
  <img src="/vairl/assets/images/banking-investor-ai-agent.svg" alt="Архитектура AI Wealth Management Platform в BlankChain" style="max-width: 100%; height: auto; display: block; margin: 0 auto 0.75em;" />
  <figcaption style="font-size: 0.9em; color: #666; max-width: 720px; margin: 0 auto;">Схема, к которой мы пришли в BlankChain: клиент → LLM+tools → оркестратор → специализированные агенты → Data Platform</figcaption>
</figure>
...
Цикл, который мы заложили в UX BlankChain:
...
```yaml
id: blankchain-explain-drawdown
...
Один «универсальный» агент в BlankChain раздувал контекст и ломал audit. Мы разрезали ответственность: Portfolio, Goals, Simulation, Recommendation, Monitoring, Compliance, Communication. Оркестратор маршрутизирует; Compliance — обязательная станция перед клиентом на pre-trade и на advice-like ответах.
...
flowchart TB
    ...
    subgraph DataPlatform ["Data Platform BlankChain"]
        CRM["CRM"]
        Core["Core Banking"]
        Broker["Brokerage"]
        Cards["Cards"]
...
Без единого профиля персонализация — театр. В BlankChain Customer 360 собирал:
...
Это разговор, который у меня чаще всего случался с product и risk в BlankChain — оставляю в формате Q&A, так честнее.
...
Самый частый вопрос от инженеров: *на чём гонять, если прод трогать нельзя?* В BlankChain первый месяц eval жил почти целиком на синтетике и публичных данных.
...
### Как мы генерили синтетику в BlankChain
...
```yaml
persona_id: blankchain-ivan-2030
...
Пока этот pack не зелёный на CI — в BlankChain мы даже не просили доступ к прод-витрине.
...
Recommendation и Simulation **не** ходили к клиенту мимо Compliance. Это не теория — это условие, на котором нам вообще дали контур в приложении BlankChain.

---

## Модели, которые реально крутятся {#math-models}

Полный энциклопедический список можно разворачивать бесконечно. В проде BlankChain «на горячем пути» чаще всего жили эти:
...
3. **Синтетику завёл бы в первую неделю.** Kaggle и persona-pack окупились быстрее, чем ожидание доступа к витринам BlankChain.  
