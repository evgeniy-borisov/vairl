---
layout: post
title: "Агент-ассистент инвестора в банке: архитектура, технологии и математика"
date: 2026-07-13 18:00:00 +0300
excerpt: "Как крупные банки строят AI Wealth Management Platform: Customer 360, мультиагентная архитектура, RAG, портфельная оптимизация и полный цикл работы с клиентом — от ретроспективы до проактивного мониторинга."
lang: ru
...
Инвестиционный ассистент в банке — это **AI Wealth Management Platform**, а не чат-бот: Customer 360, мультиагентный оркестратор, solvers для портфеля и **LLM только для объяснений** (решения — у optimizer + Compliance).

- **Цикл клиента** — ретроспектива → диагностика → сценарии → план → фоновый мониторинг с триггерами.
- **Стек** — Data Lake, Feature Store, Knowledge Graph, RAG, Kafka/CEP, Monte Carlo, Markowitz/Black–Litterman.
- **LLM не торгует** — Recommendation Agent + Compliance Agent; Communication Agent переводит цифры в диалог.
- **Классификация задач** — три оси VAIRL (таксономия → L×D → U–S–Y) маршрутизируют запрос до agent loop.
...
Инвестиционный ассистент в мобильном приложении банка — это уже не чат-бот с FAQ. Чтобы вести клиента к финансовым целям, система должна знать его портфель, доходы и расходы, семейную ситуацию, предпочтения и жизненные планы — и уметь объяснять прошлое, диагностировать настоящее, моделировать будущее, строить план и следить за отклонениями.

В JPMorgan, Morgan Stanley, Goldman Sachs, UBS, Bank of America, Сбере, Т-Банке и других крупных игроках такие системы строятся как **AI Wealth Management Platform** — композиция аналитических моделей, рекомендательных движков, LLM, RAG, event-driven архитектуры и классических банковских систем.
...
Цикл работы с клиентом разбивается на пять фаз:

| Фаза | Название | Суть |
|------|----------|------|
| 1 | **Прошлое** (ретроспектива) | Объяснение исторической доходности |
| 2 | **Настоящее** (диагностика) | Оценка текущего состояния, выявление отклонений, передача контекста в сервис портфельных рекомендаций |
| 3 | **Будущее** (прогнозирование) | Интерактивное моделирование сценариев |
| 4 | **Планирование** | Формирование инвестиционного плана с учётом прошлого, настоящего и сценариев; создание индивидуальных триггеров |
| 5 | **Мониторинг** | Сравнение факта с планом, фоновая аналитика, проактивная коммуникация |
...
- почему портфель вырос или упал;
- какие активы внесли основной вклад;
- какие решения были удачными, какие — ошибочными;
- как результат соотносится с бенчмарком и уровнем риска.

Brinson Attribution разлагает доходность на allocation effect (выбор классов активов), selection effect (выбор конкретных бумаг) и interaction effect.
...
- отклонения от целевого распределения (drift);
- ликвидность и налоговая эффективность;
- свободный денежный поток.

Результат диагностики передаётся в сервис портфельных рекомендаций как структурированный контекст — не как свободный текст.
...
- рост или падение рынков;
- изменение процентных ставок;
- покупка недвижимости;
- выход на пенсию;
- изменение доходов семьи.

Клиент видит не один прогноз, а веер сценариев с вероятностями — и может менять параметры в реальном времени.
...
- контрольные точки (milestones) достижения целей.

На этом этапе создаются **индивидуальные триггеры** — правила, которые Monitoring Agent будет отслеживать в фоне.
...
Современный AI-ассистент инвестора использует **не одну модель, а композицию из десятков**, объединённых в единый конвейер.
...
|-----|---------------|--------|
| **1. Предмет** | Пятуровневая таксономия (Domain → Model → Method) | *О чём* задача, к какой модели относится? |
...
На этапе **Contract** ([постановка задачи агента](/vairl/blog/2026/07/04/agent-task-specification-ru/)) Communication Agent restate'ит не только Y («целевая доходность 8%»), но и `abstract_model`: «понимаю, это задача goal-based investing с горизонтом 15 лет — верно?»
...
flowchart TD
  ...
  R -->|attribution, symbolic| PA["Portfolio Agent\nBrinson pipeline"]
  R -->|optimization, symbolic| RA["Recommendation Agent\nsolver"]
  R -->|simulation, hybrid| SA["Simulation Agent\nMC + Digital Twin"]
  R -->|monitoring, L4| MA["Monitoring Agent\nCEP + triggers"]
  R -->|explanation / dialogue| CA["Communication Agent\nRAG + LLM"]
