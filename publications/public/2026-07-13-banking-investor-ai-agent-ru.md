...
Формально можно писать что-то вроде:

\[
NPV_{client} = PV(\text{цель достигнута / богатство}) - PV(\text{комиссии + налоги + лишние продукты + потери от плохих решений})
\]

Банковский агент почти никогда не ставит \(PV(\text{комиссии})\) в loss с минусом для *себя*. Клиентский — обязан.
