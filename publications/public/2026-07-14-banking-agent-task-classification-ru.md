...
*Это вторая часть. Первая — [«Инвестиционный агент на стороне клиента, а не банка: DCF/NPV и конфликт интересов»](/vairl/blog/2026/07/13/banking-investor-ai-agent-ru/): две NPV (банк vs клиент) и почему objective агента должен жить у клиента.*

На проекте запрос «помоги с инвестициями» без классификатора ломал оркестратор: LLM отвечал на всё, optimizer звали на FAQ. Ниже — как размечать задачи — тот же каркас полезен и для client-side агента (с objective `client_goal_npv`).
