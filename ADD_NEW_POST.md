...
### ⚠️ Дата и время публикации — только в прошлом

Jekyll **не включает** пост в сборку, если поле `date` в frontmatter **позже** момента сборки (`site.time`). В логе: `Skipping: ... has a future date`. Статья останется в репозитории, но на сайте будет **404**.

**Правило:** `date` (дата **и** время) должны быть **уже прошедшими** на момент `git push` и прохождения CI на GitHub Actions (сборка в **UTC**).

| Ситуация | Рекомендация |
|----------|--------------|
| Публикуете сегодня | Утро текущего дня в `+0300`, например `10:00:00` |
| Push вечером по Москве | Не ставьте `21:00 +0300` «на сегодня» — CI в UTC может собрать раньше |
| Сомневаетесь | Явно прошедшее время или вчерашний день |

```yaml
# ❌ Плохо: время ещё не наступило на момент деплоя
date: 2026-07-03 18:00:00 +0300

# ✅ Хорошо: утро текущего дня (уже прошло к моменту push днём)
date: 2026-07-03 10:00:00 +0300

# ✅ Хорошо: публикация «задним числом»
date: 2026-07-02 12:00:00 +0300
```

- Публикуется поле **`date` в YAML**, а не дата в имени файла (они могут отличаться, но URL строится из `date`).
- Перед push: `bundle exec jekyll build --baseurl /vairl 2>&1 | grep -q "future date"` — вывода быть не должно.

Подробнее: [.cursor/rules/jekyll-blog-post-dates.mdc](.cursor/rules/jekyll-blog-post-dates.mdc).
...
**Дата:** указывайте дату и время **в прошлом** относительно момента деплоя (см. раздел «Дата и время публикации» выше). Формат с часовым поясом: `YYYY-MM-DD HH:MM:SS +0300`.
...
- [ ] **`date` в прошлом** (дата и время ≤ момент push / CI в UTC) — иначе Jekyll пропустит пост
- [ ] `bundle exec jekyll build` без `has a future date` в логе
