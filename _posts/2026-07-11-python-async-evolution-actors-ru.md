...
## Сети Петри: формальная модель конкурентности агентов

Практические примеры выше — sync, threads, asyncio, акторы — описывают *механизмы* исполнения. **Сеть Петри** (Petri net) даёт *математическую* модель: что именно может выполняться одновременно, какие ресурсы расходуются и где система может зависнуть. Для LLM-агентов это особенно полезно: DAG оркестратора, очередь задач, семафор провайдера и бюджет контекста — всё это укладывается в одну схему с **фишками** (tokens), которые передаются между **позициями** через **переходы**.

### Базовые элементы

| Элемент | В теории | В agent platform |
|---------|----------|------------------|
| **Позиция (place)** | Состояние системы; в ней лежат фишки | «Задача готова», «ждёт LLM», «слот семафора свободен», «бюджет токенов остался» |
| **Переход (transition)** | Событие; срабатывает, если хватает входных фишек | `call_llm`, `await_tool`, `aggregate`, захват/освобождение слота |
| **Фишка (token)** | Дискретная единица ресурса или работы | Единица задачи, слот concurrency, единица rate-limit |
| **Маркировка (marking)** | Распределение фишек по позициям *сейчас* | Снимок состояния оркестратора в момент времени |

Переход **срабатывает** (fires), когда во всех входных позициях достаточно фишек; после срабатывания фишки снимаются с входов и кладутся на выходы. Это строгое правило конкурентности: два перехода, конкурирующие за один и тот же слот, не могут сработать одновременно — ровно как `Semaphore(3)` не пустит четвёртый параллельный вызов LLM.
...
flowchart TB
  subgraph legend["Легенда"]
    ...
    PL["○ позиция"]
    TR["▭ переход"]
    TK["● фишка = единица ресурса"]
  end

  subgraph agent_net["Упрощённая сеть: fan-out из трёх tool calls"]
    R["ready<br/>● ● ●"]
    SL["provider_slots<br/>● ●"]
    L["waiting_llm"]
    S["waiting_search"]
    C["waiting_calendar"]
    A["aggregating"]
...
На схеме три фишки в `ready` — три независимые ветви; две фишки в `provider_slots` — семафор «не более двух одновременных вызовов к провайдеру». Пока слотов нет, переход `call_*` не срабатывает: задача *формально* ждёт ресурс, а не просто «висит в asyncio».

### Три смысла слова «token»

В агентных системах слово *token* встречается в трёх разных, но связанных смыслах. Сеть Петри помогает их не смешивать:

| Смысл | Что это | Аналог в сети Петри |
|-------|---------|---------------------|
| **Фишка Петри** | Дискретная единица *управления потоком* или *ёмкости* | Фишка в позиции `provider_slots`, `queue_capacity`, `ready` |
| **Слот concurrency** | Разрешение на параллельный вызов; реализуется `Semaphore`, thread pool, rate limiter | Ограниченное число фишек в одной позиции-«кошельке» |
| **LLM-token** | Подсловная единица текста; расходуется из **контекстного бюджета** prompt + completion | *Вес* перехода: `call_llm` потребляет `k` фишек из позиции `context_budget` |

Последняя строка — мост между формальной теорией и экономикой агента. Переход «вызвать LLM с retrieval-контекстом на 8k токенов» можно моделировать как: снять одну фишку «задача готова», снять 8000 фишек из `context_budget` (или одну фишку номинала 8k), положить фишку в `waiting_llm`. Когда провайдер ответил — вернуть остаток бюджета и передать результат дальше по графу. Так **backpressure по токенам** и **backpressure по concurrency** становятся одним языком.

### Связь с LLM-агентами и asyncio

Agent pipeline — это не просто DAG в смысле «узлы и рёбра», а **управляемая сеть** с ограниченными ресурсами:

- **Оркестратор** — движок, который ищет *разрешённые* переходы (как event loop ищет готовые корутины после `await`).
- **Корутина на `await`** — переход «запущен»: фишка ушла из `ready` в `in_flight`; по завершении I/O переход «завершён» возвращает фишку в downstream-позицию.
- **Actor mailbox** — позиция с очередью фишек-сообщений; переход «обработать сообщение» срабатывает строго по одному, пока актор не освободится.
- **Process pool** — отдельная подсеть с фишками «свободное ядро»; CPU-переход не fires без такой фишки.

Иными словами: `asyncio`, threads и actors — *реализации*; сеть Петри — *спецификация*, которую можно проверить до продакшена.

### Анализ программ и оркестраторов

Сети Петри изучают не только «как нарисовать», но и **свойства**, важные для надёжности:

| Свойство | Вопрос | Зачем агенту |
|----------|--------|--------------|
| **Достижимость (reachability)** | Может ли система попасть в данную маркировку? | «Можем ли мы теоретически исчерпать все слоты и зависнуть с незакрытым запросом?» |
| **Ограниченность (boundedness)** | Есть ли потолок числа фишек в каждой позиции? | Очередь без bound → переполнение памяти при fan-out |
| **Живучесть (liveness)** | Может ли каждый переход когда-нибудь снова сработать? | Нет — «вечная блокировка» одной ветви |
| **Deadlock** | Маркировка, где ни один переход не fires | Классический deadlock семафора + циклических зависимостей между tools |

Перед нагрузочным тестом полезно построить малую сеть для критичного сценария — например, planner → 3 tools → aggregator с семафором 2 — и проверить, что при любой длине очереди число фишек в `queued` ограничено, а `done` достижимо. Это дешевле, чем ловить deadlock в проде на 10 000 concurrent sessions.

Минимальный симулятор на Python показывает правило firing:
...
def enabled(marking: dict[str, int], transition: Transition) -> bool:
    return all(marking.get(place, 0) >= need for place, need in transition.inputs.items())


def fire(marking: dict[str, int], transition: Transition) -> dict[str, int]:
    if not enabled(marking, transition):
        raise ValueError(f"transition {transition.name!r} is not enabled")
    next_marking = dict(marking)
    for place, need in transition.inputs.items():
        next_marking[place] -= need
    for place, gain in transition.outputs.items():
        next_marking[place] = next_marking.get(place, 0) + gain
    return next_marking


# Две задачи, один слот провайдера, бюджет контекста убывает при вызове LLM
marking = {"ready": 2, "provider_slots": 1, "context_budget": 16_000, "done": 0}

start_llm = Transition(
    "start_llm",
    inputs={"ready": 1, "provider_slots": 1, "context_budget": 4_000},
    outputs={"in_flight": 1},
)
finish_llm = Transition(
    "finish_llm",
    inputs={"in_flight": 1},
    outputs={"provider_slots": 1, "done": 1},
)

marking = fire(marking, start_llm)
print("after start:", marking)  # ready=1, in_flight=1, slots=0, budget=12000
marking = fire(marking, finish_llm)
print("after finish:", marking)  # done=1, slots=1 — второй вызов снова может стартовать
```

Здесь `context_budget` играет роль позиции с **LLM-токенами как ресурсом**: переход `start_llm` не сработает, если бюджета не хватает — аналог отказа planner'а отправить слишком длинный prompt.

### Практический вывод

Сеть Петри не заменяет профилирование и метрики, но даёт **общий формализм** для трёх вещей, которые в статье уже фигурировали отдельно: конкурентность ветвей DAG, ограничение параллелизма (семафор, pool) и расход контекста LLM. Если нарисовать агентный pipeline как Petri net, вопросы собеседования и code review становятся конкретнее: где bound очереди, что happens при исчерпании слотов, может ли fan-out создать deadlock, кто владеет фишками состояния сессии. Aktor с mailbox — частный случай; `TaskGroup` с semaphore — тоже; разница только в том, *как* движок ищет разрешённые переходы. Подробный разбор production coding agents (Claude Code, Cursor, Codex, OpenCode, Aider) и минимальный harness на Python — в [отдельной секции про архитектуры агентов](/vairl/blog/2026/07/10/python-async-evolution-coding-agents-ru/).
...
- Как сеть Петри помогает увидеть deadlock и исчерпание слотов до продакшена?
