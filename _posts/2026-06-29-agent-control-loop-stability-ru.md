...
У AI-агента возмущение — не только шум датчика. Это **галлюцинация модели**, **битый tool call**, **устаревший контекст в RAG**, **изменение API** или **пользователь, который меняет цель на полпути**. Карта компетенций, в которую вписывается этот материал — в публикации [Как готовить ведущего специалиста по AI-агентам](/vairl/blog/2026/06/29/best-ai-agent-specialist-ru/).
...
## Политики обучения: где сигнал на траектории

На этапе **RL-выравнивания** (PPO, GRPO) политика $\pi_\theta$ тоже описывается траекторией в embedding space — только эту траекторию мы **сознательно сдвигаем** градиентом. Путаница частая: кажется, что PPO «оценивает ризонинг по ходу», а GRPO — «только финальный ответ». **Почти так, но с важной поправкой.**

### Уточнение: верификатор всё равно в конце

И в **PPO**, и в **GRPO** scalar-награда от RM или verifiable checker (0/1 за математику, unit test за код) в типичном RLVR-пайплайне приходит **на последнем токене** полного completion — включая CoT и reasoning-токены. **Отдельной пошаговой проверки каждого шага ризонинга** в обоих случаях нет, если вы явно не ввели process reward.

Разница не в том, *когда* внешний судья смотрит на ответ, а в том, **как сигнал распределяется по токенам при обновлении весов**:

| | PPO (actor-critic) | GRPO (group baseline) |
|---|-------------------|------------------------|
| **Rollout на промпт** | Обычно **1** траектория | **G** полных траекторий (4–64) |
| **Где r** | Terminal (конец completion) | Terminal для каждого $o_i$ |
| **Credit assignment** | **Потокенный**: критик $V(s_t)$ + GAE → разный $A_t$ на каждом токене | **Outcome**: один $A_i$ на **весь** ответ $o_i$ |
| **Baseline** | Обучаемый критик | $\mathrm{mean}(r)$ по группе на том же $q$ |

**PPO** не «судит ризонинг в реальном времени» — критик **учится предсказывать** ожидаемую ценность частичного префикса и через GAE **размазывает** terminal reward назад по цепочке. **GRPO** честнее в этом смысле: после групповой нормализации **все токены одного ответа получают одинаковый градиент** — хороший или плохой trace целиком.

Подробный разбор формул и архитектуры — в [GRPO vs PPO](/vairl/blog/2026/07/01/grpo-vs-ppo-llm-alignment-ru/).

### Интерактив: два режима credit assignment

Переключите **PPO / GRPO** и нажимайте **«Следующий шаг»**: сверху — траектории в проекции embedding space; снизу — ось времени CoT → ответ. Обратите внимание: **зелёная/красная награда r всегда в конце**; отличается только то, что происходит *после* неё.

<div id="rl-policy-credit-widget" class="rl-policy-credit-widget">
  <div class="rl-credit-header">
    <p>Где обучающий сигнал «цепляется» за траекторию: PPO — локально по токенам (GAE); GRPO — единым весом на весь trace после сравнения с группой.</p>
  </div>
  <div class="rl-credit-controls">
    <button type="button" data-credit-mode="ppo" class="active">PPO</button>
    <button type="button" data-credit-mode="grpo">GRPO</button>
    <button type="button" class="rl-credit-step-btn" data-credit-step>Следующий шаг →</button>
  </div>
  <div class="rl-credit-canvas-wrap">
    <canvas id="rl-policy-credit-canvas"></canvas>
  </div>
  <div class="rl-credit-legend"></div>
  <p class="rl-credit-caption"></p>
</div>

<script src="{{ '/assets/js/rl-policy-credit.js' | relative_url }}"></script>

### Сводка: стоимость и вычислительная сложность

Экономический компромисс — зеркало компромисса «память vs генерация» в control loop обучения:

| Экономический критерий | PPO (классика, в т.ч. GLM-стек) | GRPO (DeepSeek, RLVR) |
|------------------------|----------------------------------|------------------------|
| **Затраты на VRAM** | Высокие: критик удваивает (или существенно увеличивает) память под базовую модель | Ниже: отдельного критика нет |
| **Стоимость генерации (rollout)** | Ниже: **1** ответ на промпт за шаг обновления | Выше: **G** полных completions на каждый промпт |
| **Зависимость от длины контекста** | Близка к **линейной**: один длинный trace | **Мультипликативная**: $G \times L$ — группа длинных CoT (10k+ токенов) резко бьёт по FLOPs |
| **Скорость схождения** | Часто быстрее на задачах с длинным credit path и shaped reward | Быстро на коротких верифицируемых задачах (math); на длинных агентских сценариях группа может не дать тонкого сигнала |

Зависимость GRPO от длины — не буквально «экспоненциальная», а **произведение** числа сэмплов на длину каждого; на практике при reasoning 10k+ токенов это ощущается как взрывной рост счёта за обучение.

### Фазовый портрет inference vs фазовый портрет обучения

Это **два разных среза** одной геометрии — их полезно не смешивать:

```mermaid
flowchart TB
  subgraph infer["Inference (runtime)"]
    I1[Траектория в embedding space] --> I2[Аттракторы / циклы / сепаратрисы]
    I2 --> I3[Диагностика устойчивости агента]
  end
  subgraph train["Training (RL)"]
    T1[Траектории rollout] --> T2[Terminal r + credit assignment]
    T2 --> T3[Сдвиг векторного поля π_θ]
  end
  infer -. "одна сессия, без градиента" .-> train
  train -. "меняет поле для будущих сессий" .-> infer
```

| Аспект фазового портрета | Во время работы агента | Во время RL-обучения |
|--------------------------|------------------------|----------------------|
| **Что за траектория** | Один trace на задачу пользователя | Множество rollouts на обучающих промптах |
| **Кто двигает точку** | Политика + среда + tools | Policy gradient по награде |
| **PPO-аналог** | Локальные «стрелки» поля меняются от шага к шагу (разный grounding на каждом hop) | GAE даёт **разный толчок** на ранних и поздних токенах — **локальное искривление** траекторий в пространстве политики |
| **GRPO-аналог** | Несколько параллельных попыток на один запрос (best-of-N) | Целые траектории **ранжируются** относительно группы — **сдвиг бассейнов** «весь trace выше/ниже среднего» |
| **Риск неустойчивости** | Limit cycle, расходимость, суперкритичность CoT | Ошибочный критик (PPO) или $\sigma \to 0$ в группе (GRPO) — положительная обратная связь в **обучающем** контуре |

**Практический вывод:** фазовый портрет из разделов выше — инструмент **мониторинга runtime**. Таблица PPO/GRPO — про **как вы платите**, чтобы **перекроить** это поле на offline-данных. Длинный агентский DAG с редким terminal success ближе к боли GRPO (мало контраста в группе, дорогие rollouts); задачи с плотным промежуточным сигналом или необходимостью точного потокенного кредита — к сильным сторонам PPO, ценой VRAM.
...
- [Как готовить ведущего специалиста по AI-агентам](/vairl/blog/2026/06/29/best-ai-agent-specialist-ru/) — полная карта компетенций
...
- [GRPO vs PPO: выравнивание LLM](/vairl/blog/2026/07/01/grpo-vs-ppo-llm-alignment-ru/) — формулы, VRAM, стабильность обучающего контура
