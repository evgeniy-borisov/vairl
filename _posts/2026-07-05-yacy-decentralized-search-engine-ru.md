...
| [Свои файлы](#индексация-своих-файлов) | Intranet, краулинг, surrogates, API |
...
## Индексация своих файлов

YaCy **не индексирует пути вида** `/home/user/Documents` напрямую. Краулер работает с **URL**: HTTP, HTTPS, FTP, SMB (Samba). Для личных файлов, корпоративного NAS или архива PDF есть три семейства подхода:

1. **Отдать файлы как веб-каталог** и прокраулить.
2. **Импортировать метаданные** в каталог `DATA/SURROGATES/in`.
3. **Залить документы через API** `/api/push_p.json`.

Ниже — пошаговая настройка **приватного** поиска только по своим данным (без утечки в freeworld).

### Шаг 0. Режим Intranet и изоляция от P2P

Перед индексацией закройте индекс от глобальной сети:

| Шаг | Где | Что включить |
|-----|-----|--------------|
| 1 | [`/ConfigBasic_p.html`](http://localhost:8090/ConfigBasic_p.html) | Use case: **Intranet** |
| 2 | [`/ConfigAccounts_p.html`](http://localhost:8090/ConfigAccounts_p.html) | Пароль `admin` |
| 3 | [`/ConfigNetwork_p.html`](http://localhost:8090/ConfigNetwork_p.html) | **Robinson Mode** → `privatepeer` (нет обмена с freeworld) |
| 4 | Там же | **Index Distribution** = OFF, **Index Receive** = OFF |
| 5 | [`/CrawlStart_p.html`](http://localhost:8090/CrawlStart_p.html) | При каждом крауле: **Do Remote Indexing** = OFF |

После этого RWI **не уходит** в DHT, индекс остаётся только в локальном Solr. Порт 8090 наружу можно **не открывать** — Senior-режим для личного архива не нужен.

Официальная шпаргалка: [Setup Intranet](https://wiki.yacy.net/index.php/En:SetupIntranet).

```mermaid
flowchart LR
    FILES[Ваши файлы] --> HTTP[HTTP каталог или API]
    HTTP --> YACY[YaCy crawl / import]
    YACY --> SOLR[Локальный Solr]
    SOLR --> SEARCH[Поиск localhost 8090]
```

### Способ 1. Каталог файлов через HTTP (рекомендуется)

Самый практичный путь: поднять лёгкий веб-сервер с листингом директорий и отдать YaCy URL корня.

**Пример: nginx + autoindex**
...
**Пример: lighttpd** (часто на NAS / домашнем сервере) — каталог монтируется в `document-root`, YaCy краулит `http://192.168.1.10:8081/`.

**Запуск краула** — Production → [Advanced Crawler](http://localhost:8090/CrawlStart_p.html):

| Параметр | Рекомендация для своих файлов |
|----------|-------------------------------|
| **From URL** | `http://192.168.1.10:8081/` (корень архива) |
| **Crawling Depth** | `0` = только стартовая страница; `2–4` = обход вложенных ссылок; для полного дерева — **Unlimited crawl depth** с фильтром на свой URL |
| **Restrict to sub-path** | ✅ — не уйти на внешние сайты из PDF/Word |
| **Must-Match filter** | `http://192\.168\.1\.10:8081/.*` |
| **Must-Not-Match** | `.*amazon\.|.*google\.` — блок внешних доменов из гиперссылок в документах |
| **Do Local Indexing** | ✅ |
| **Do Remote Indexing** | ❌ |
| **Re-crawl known URLs** | ✅ + интервал — для ежедневного обновления |

**Глубина обхода:** если видите только 2–3 уровня папок — увеличьте depth или включите unlimited depth **только** для своего префикса URL ([обсуждение на форуме](https://community.searchlab.eu/t/change-crawl-depth-on-intranet/948)).

**Поддерживаемые типы** при HTTP-краулинге: HTML, **PDF** (встроенный парсер; постраничное разбиение — `parser.pdf.individualpages=true` в `yacy.conf`), Office через конвертацию в HTML на стороне сервера, plain text. Бинарники без текста (jpg, zip) индексируются слабо — только метаданные URL.

### Способ 2. Свой сайт, wiki или Git-страницы

Если файлы уже на веб-сервере (GitLab Pages, внутренний Confluence, статический сайт):

```
From URL: https://docs.mycompany.local/
Restrict to start domain: ✅
Crawling Depth: 3–5
Do Remote Indexing: ❌
```

Для **одного сайта** без P2P достаточно: DHT выключен, краул только вашего домена.

### Способ 3. Surrogates — импорт без краулинга

Каталог **`DATA/SURROGATES/in/`** — «приёмник» XML-документов. YaCy подхватывает файлы **в фоне**, пока работает; перезапуск не нужен.

**Dublin Core XML** (минимальный пример):

```xml
<?xml version="1.0" encoding="utf-8"?>
<surrogates xmlns:dc="http://purl.org/dc/elements/1.1/">
  <record>
    <dc:title>Отчёт Q1 2026</dc:title>
    <dc:identifier>file:///archive/reports/q1-2026.pdf</dc:identifier>
    <dc:description><![CDATA[Квартальный отчёт: выручка, метрики, план.]]></dc:description>
...
Положите файл в `DATA/SURROGATES/in/report.xml` — через несколько секунд запись появится в индексе.

**Также принимается:**

- экспорт Solr XML из другого YaCy ([Index Export](http://localhost:8090/IndexExport_p.html) → файл в `SURROGATES/in`);
- WARC-архивы (из `wget --warc-file`; нужна свежая сборка YaCy);
- пакетная генерация XML скриптом из каталога PDF/Markdown.

Подробнее: [Index export and import](https://yacy.net/operation/index-export-import/).

### Способ 4. API push — массовая загрузка

Для пайплайна «скрипт обходит диск → пушит в YaCy»:

```
POST http://localhost:8090/api/push_p.json
Content-Type: multipart/form-data
```

Параметры: бинарные поля с документами, `count`, `synchronous=false`, `commit=false` для максимальной скорости. Документы проходят через **встроенные парсеры** (PDF, HTML и др.) и попадают в Solr.

Документация: [API push](https://wiki.yacy.net/index.php/Dev:APIpush).

Типичный сценарий: ночной cron на сервере с архивом — обходит новые файлы, пушит только изменённые.

### Способ 5. HTML-файл со списком ссылок

В Advanced Crawler → **From File**: HTML с `<a href="...">` на каждый документ. YaCy обойдёт ссылки с depth `1`. Удобно, если скрипт сгенерировал `links.html` из `find /archive -name '*.pdf'`.

Для десятков тысяч URL разбейте список: `split -l 10000 links.html chunk_`.

### Способ 6. HTTP-прокси (только просмотренное)

YaCy может работать как **прокси**: индексирует страницы, которые вы открываете в браузере. Включение: Settings → Proxy. Ограничение: **HTTPS** в современных браузерах почти не проксируется прозрачно — для архива файлов способ 1 или 3 надёжнее.

### Ежедневное обновление и удаление устаревшего

| Задача | Решение |
|--------|---------|
| Переобход раз в день | Bookmark в папке `/autoReCrawl/daily` при старте краула; настройки в `DATA/SETTINGS/autoReCrawl.conf` |
| Переиндексировать известные URL | **Re-crawl known URLs** в профиле краула |
| Удалить тип файлов из индекса | [Index Deletion](http://localhost:8090/IndexDeletion_p.html) — regex, напр. `.*\.png` |
| Удалить всё и начать заново | Cleanup → Delete RWI / Solr (осторожно, необратимо) |

Автоматического recrawl «само по себе» для всего индекса **нет** — расписание нужно настроить явно.

### Docker: локальный архив
...
docker run -d \
  ...
  -v /home/user/archive:/archive:ro \
  --restart unless-stopped \
  yacy/yacy_search_server:latest
```

Файлы в `/archive` YaCy **не увидит**, пока не смонтируете рядом nginx или не импортируете через `SURROGATES/in` / API push. Типичная схема: два контейнера — `nginx:alpine` (autoindex на volume) + YaCy.

### Поиск только по своему индексу

После настройки Intranet + privatepeer:

- в поиске выбирайте **локальный индекс** (без глобального DHT);
- в Debug/Analysis можно отключить поиск в RWI freeworld;
- UI: `http://localhost:8090/yacysearch.html` — запросы не уходят наружу.

### Чеклист «личный поиск по файлам»

- [ ] Режим **Intranet**, Robinson **privatepeer**
- [ ] DHT Distribution / Receive — **выключены**
- [ ] Пароль admin установлен
- [ ] Файлы доступны по **HTTP** или импортированы в **SURROGATES/in**
- [ ] Краул: **Do Remote Indexing** выключен, фильтр **restrict to sub-path**
- [ ] Расписание **autoReCrawl** (если архив меняется)
- [ ] Порт 8090 **не** проброшен в интернет (для приватного архива)

Это **лучший сценарий** YaCy: полнотекстовый поиск по своим данным без Google, без облака и без утечки индекса в freeworld.
